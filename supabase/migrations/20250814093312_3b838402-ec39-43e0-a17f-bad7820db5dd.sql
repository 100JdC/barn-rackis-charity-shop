-- Security improvements: Add RLS policies to public views and fix database function security

-- Add RLS policies to public views to prevent unauthorized access
ALTER TABLE public.items_public_secure ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.items_public_view ENABLE ROW LEVEL SECURITY;

-- Policy for items_public_secure: Only show available items
CREATE POLICY "Public can view available items secure" 
ON public.items_public_secure
FOR SELECT 
USING (Status = 'available');

-- Policy for items_public_view: Only show available items
CREATE POLICY "Public can view available items" 
ON public.items_public_view
FOR SELECT 
USING (Status = 'available');
